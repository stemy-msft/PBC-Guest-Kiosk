...
def generate_visitor_badge(visitor, output_path: Path) -> Path:
    ...
    full_name = f"{visitor.first_name} {visitor.last_name}".strip().upper()
    name_font = _fit_text(
        draw=draw,
        text=full_name,
    ...
    draw.text((text_x, 64), full_name, fill=FOREGROUND, font=name_font)

    type_font = _font(25, bold=True)
    draw.text(
        (text_x, 112),
    ...
    value_font = _font(21, bold=False)
    ...
    _draw_label(
        draw=draw,
        xy=(text_x, 180),
        label=visitor.host_type,
        value=visitor.host_name,
        label_font=label_font,
        value_font=value_font,
    )

    check_font = _font(18, bold=True)
